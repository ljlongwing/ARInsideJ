...
#
# This image is NOT built from source. ARInsideJ needs the BMC AR System Java API jars
# (arapi / arlogger) to compile, and those are not on any public Maven repository, so an
# image build has nothing to resolve them from. Instead this wraps a pre-built
# self-contained fat jar (target/arinsidej.jar), which bundles all of its dependencies.
#
